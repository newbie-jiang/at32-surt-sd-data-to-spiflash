...
//  crm_periph_clock_enable(CRM_XMC_PERIPH_CLOCK, TRUE);
  ...
  gpio_bits_write(GPIOE,GPIO_PINS_4,false);
...
#define ZQ_FLASH_CS_GPIO_PORT    GPIOE


void spiflash__init(void)
{
  gpio_init_type gpio_initstructure;
  spi_init_type spi_init_struct;
  /* 使能对应端口的时钟 */
...
{
  FLASH_CS_LOW();
  spi_byte_write(SPIF_RELEASEPOWERDOWN);
  FLASH_CS_HIGH();
}
